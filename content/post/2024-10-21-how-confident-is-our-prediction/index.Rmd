...
Our [previous working model](/usa-2024-election-prediction/content/post/2024-10-14-campaign-ad-spending) only used data from competitive states. Here we will proceed with the 7 swing states. Can we improve our prediction accuracy by combining data from non-swing states?
...
swing_interval_df <- swing_interval_df %>% mutate(State = state_names, Source = "Swing States")
...
ggplot(combined_df, aes(x = State, y = pred, color = Source)) +
  ...
  labs(title = "Prediction Intervals for Swing States",
       x = "State", 
...
Harris wins with more than 50% occurence in Maine, Michigan, Minnesota, Nevada, and New Hampshire.

How about the electoral collage?
```{r echo=FALSE}
...
Harris will win 40% of the time. The reader might be curious how this can be different from a predicted Harris win last time. This relates to how the mean and the median are different objects. The previous presentation relies more on the mean, and this presentation relies more on the median.

I also want to note that this model predicts that Harris has a 1/3 chance of winning Iowa, and the probability of the other non-swing states being won is not close to 90%. This is different from prevailing judgement and the betting market, so we might have to tweak the model further to include state-level approval for Harris.
