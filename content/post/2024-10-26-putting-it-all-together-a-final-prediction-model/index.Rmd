...
lm_loo_errors <- sapply(1:nrow(selected), function(i) {
  # Fit the model excluding observation i
  lm_loo <- ranger(pv2p ~ juneapp + q2_gdp_growth + incumbent, data = selected[-i, ])
  # Predict the excluded observation
  prediction <- predict(lm_loo, data = selected[i, ])$predictions
  # Calculate the squared error for the excluded observation
  (selected$pv2p[i] - prediction)^2
})
...
for (pred_yr in YEARS) {
  ...
  model <- ranger(formula = inc_pv2p ~ inc_poll2p + q2_gdp_growth + incumbent,
              data = train_data)
...
for (pred_yr in YEARS) {
  ...
  # Fit the model using training data
  model <- ranger(formula = inc_pv2p ~ inc_poll2p + q2_gdp_growth + incumbent  + inc_lag + swing,
              data = train_data)
...
```{r}
model <- ranger(formula = inc_pv2p ~ inc_poll2p + q2_gdp_growth + incumbent  + inc_lag + swing, data =  state_popvote_gdp_polls)

data2024$preds <- predict(model, data2024)$predictions
data2024
```
...
Our final prediction gives a 32.6% chance of a Harris win.
...
for (pred_yr in YEARS) {
  ...
  # Fit the model using training data
  model <- ranger(formula = inc_pv2p ~ inc_poll2p + q3_gdp_growth + incumbent  + inc_lag + swing,
              data = train_data)
...
for (pred_yr in YEARS) {
  ...
  # Fit the model using training data
  model <- ranger(formula = inc_pv2p ~inc_poll2p + q2_gdp_growth + incumbent  + inc_lag + swing+ deminc,
              data = train_data)
...
for (pred_yr in YEARS) {
  ...
  # Fit the model using training data
  model <- ranger(formula = inc_pv2p ~inc_poll2p + q2_gdp_growth_4y + incumbent  + inc_lag + swing,
              data = train_data)
...
for (pred_yr in YEARS) {
  ...
  # Fit the model using training data
  model <- ranger(formula = inc_pv2p ~ inc_poll2p + q2_gdp_growth + incumbent  + inc_lag + swing,
              data = filter(train_data, year!=2020) )
